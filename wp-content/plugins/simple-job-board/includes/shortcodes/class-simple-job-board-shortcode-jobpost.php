...
class Simple_Job_Board_Shortcode_Jobpost {
     ...
     */
    public function jobs_listing($atts, $content) {
        ...
        $args['jobpost_category'] = (isset($_GET['selected_category']) && -1 != $_GET['selected_category']) ? esc_attr($_GET['selected_category']) : esc_attr($shortcode_args['category']);
        $args['jobpost_job_type'] = (isset($_GET['selected_jobtype']) && -1 != $_GET['selected_jobtype']) ? esc_attr($_GET['selected_jobtype']) : esc_attr($shortcode_args['type']);
        $args['jobpost_location'] = (isset($_GET['selected_location']) && -1 != $_GET['selected_location']) ? esc_attr($_GET['selected_location']) : esc_attr($shortcode_args['location']);
         ...
         */
        return apply_filters('sjb_job_listing_shortcode', $html . do_shortcode($content));
    }
